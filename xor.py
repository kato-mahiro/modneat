...
for g in range(5000):
    for n in range(10):
        error = 0.0
        error += (a[n].get_output_without_update(inputs[0])[0] - outputs[0]) ** 2
        error += (a[n].get_output_without_update(inputs[1])[0] - outputs[1]) ** 2
        error += (a[n].get_output_without_update(inputs[2])[0] - outputs[2]) ** 2
        error += (a[n].get_output_without_update(inputs[3])[0] - outputs[3]) ** 2
        a[n].fitness = error * -1
    if(g % 100 == 0):
